...
{ 
    ...
    public class Upload
    {
        ...
        static int NumBytesPerChunk = 512 * kB; // A block may be up to 4 MB in size. 

        public static void Upload(string inputFile, string containerName, string storageConnectionString)
        {
            upload(new FileInfo(inputFile), CloudStorageAccount.Parse(storageConnectionString), containerName);
        }
...
[LargeFileUploader.Upload]::upload("
